...
// 取用顺序与生效预览：页面在语言区下面读取与整体保存这条顺序
app.get('/api/fallback-order', (_req, res) => {
  res.json(api.listFallbackOrder());
});

app.put('/api/fallback-order', (req, res) => {
  try {
...
function sendError(res, err) {
  ...
  console.error('[tp70] 处理请求时出现未预期的问题：', err);
